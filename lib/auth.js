...
if (nconf.get('redis-uri')) {
    uri = url.parse(nconf.get('redis-uri'));
...
exports.session = function (request, response, next) {
    ...
    client.get(token, function (error, id) {
        if (error) {return response.send(500, error);}
        if (!id) {return response.send(401, 'invalid token');}
