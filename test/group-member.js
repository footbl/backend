...
describe('group controller', function () {
    ...
    describe('create', function () {
        describe('free to edit', function () {
            ...
            it('should raise error with invalid group id', function (done) {
                ...
                credentials = auth.credentials();
                req = request(app);
                req = req.post('/groups/invalid/members');
                req = req.set('auth-signature', credentials.signature);
                req = req.set('auth-timestamp', credentials.timestamp);
                req = req.set('auth-transactionId', credentials.transactionId);
                req = req.set('auth-token', auth.token(groupOwner));
            ...
            it('should create with user token', function (done) {
                ...
                req = req.send({'group' : slug});
                req = req.send({'user' : anotherGroupUser.slug});
                req.expect(201);
                req.expect(function (response) {
                    response.body.should.have.property('user');
                    response.body.should.have.property('slug');
    ...
    describe('details', function () {
        ...
        it('should raise error without token', function (done) {
            ...
            credentials = auth.credentials();
            req = request(app);
            req = req.get('/groups/' + slug + '/members/' + groupUser.slug);
            req = req.set('auth-signature', credentials.signature);
            req = req.set('auth-timestamp', credentials.timestamp);
            req = req.set('auth-transactionId', credentials.transactionId);
            req.expect(401);
        ...
        it('should raise error with invalid group id', function (done) {
            ...
            credentials = auth.credentials();
            req = request(app);
            req = req.get('/groups/invalid/members/' + groupUser.slug);
            req = req.set('auth-signature', credentials.signature);
            req = req.set('auth-timestamp', credentials.timestamp);
            req = req.set('auth-transactionId', credentials.transactionId);
            req = req.set('auth-token', auth.token(groupOwner));
        ...
        it('should raise error with invalid id', function (done) {
            ...
            credentials = auth.credentials();
            req = request(app);
            req = req.get('/groups/' + slug + '/members/invalid');
            req = req.set('auth-signature', credentials.signature);
            req = req.set('auth-timestamp', credentials.timestamp);
            req = req.set('auth-transactionId', credentials.transactionId);
            req = req.set('auth-token', auth.token(groupOwner));
    ...
    describe('update', function () {
        describe('free to edit', function () {
            ...
            it('should raise without token', function (done) {
                ...
                credentials = auth.credentials();
                req = request(app);
                req = req.put('/groups/' + slug + '/members/' + groupUser.slug);
                req = req.set('auth-signature', credentials.signature);
                req = req.set('auth-timestamp', credentials.timestamp);
                req = req.set('auth-transactionId', credentials.transactionId);
                req = req.send({'user' : anotherGroupUser.slug});
            ...
            it('should raise with invalid group id', function (done) {
                var req, credentials;
                credentials = auth.credentials();
                req = request(app);
                req = req.put('/groups/invalid/members/' + groupUser.slug);
                req = req.set('auth-signature', credentials.signature);
                req = req.set('auth-timestamp', credentials.timestamp);
                req = req.set('auth-transactionId', credentials.transactionId);
                req = req.set('auth-token', auth.token(groupOwner));
            ...
            it('should raise with invalid id', function (done) {
                ...
                credentials = auth.credentials();
                req = request(app);
                req = req.put('/groups/' + slug + '/members/invalid');
                req = req.set('auth-signature', credentials.signature);
                req = req.set('auth-timestamp', credentials.timestamp);
                req = req.set('auth-transactionId', credentials.transactionId);
                req = req.set('auth-token', auth.token(groupOwner));
            ...
            it('should update with user token', function (done) {
                ...
                credentials = auth.credentials();
                req = request(app);
                req = req.put('/groups/' + slug + '/members/' + anotherGroupUser.slug);
                req = req.set('auth-signature', credentials.signature);
                req = req.set('auth-timestamp', credentials.timestamp);
                req = req.set('auth-transactionId', credentials.transactionId);
                req = req.set('auth-token', auth.token(groupUser));
